...
def render_sets(dataset : ModelParams,
    ...
    with torch.no_grad():
        ...
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, skip_ply=True)
        ...
        for model in args.models:
            ...
            try:
                scene.gaussians.load_ply_reduce(os.path.join(scene.model_path,
                                                            "point_cloud",
                                                            "iteration_" + str(scene.loaded_iter),
                                                            name), quantised=quantised, half_float=half_float, variable_sh_bands = False)
            except:
