	...
	)
{
	...
	for (int i = 0; i < cam_positions.size(0); ++i)
	{
		...
		binningFunc,
		imgFunc,
	    P, max_sh_deg, degrees.contiguous().data<int>(), M,
		...
		cam_viewmatrices.contiguous().data<float>(), 
		cam_projmatrices.contiguous().data<float>(),
		cam_positions.contiguous().data<float>(),
		tan_fovx,
		tan_fovy,
		false,
		...
		printf("out_transmittance is ", out_transmittance);
        calculateColour(P,
