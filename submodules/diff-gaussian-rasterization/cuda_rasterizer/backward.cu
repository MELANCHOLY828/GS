...
__device__ void computeColorFromSH_reduce(int idx, int* degs, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	// Compute intermediate values, as it is done during forward
	glm::vec3 pos = means[idx];
	glm::vec3 dir_orig = pos - campos;
	...
	glm::vec4* dL_drot)
{
	...
	if (shs)
	{
		...
		computeColorFromSH_reduce(idx, Ds, M, (glm::vec3*)means, *campos, shs, clamped, (glm::vec3*)dL_dcolor, (glm::vec3*)dL_dmeans, (glm::vec3*)dL_dsh);
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// atomicAdd(&dL_dmean2D[global_id].z, fabs((dL_dG * dG_ddelx + dL_ddepth * depth_plane.x) * ddelx_dx));
			// atomicAdd(&dL_dmean2D[global_id].w, fabs((dL_dG * dG_ddely + dL_ddepth * depth_plane.y) * ddely_dy));
