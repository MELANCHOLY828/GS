...
output_dir = "/data/liufengyi/res_gs/AtomGS_all_sky_eval"
jobs = list(zip(scenes, factors, normal_weights))


def train_scene(gpu, scene, factor, weight):
    cmds = [
            f"CUDA_VISIBLE_DEVICES={gpu} python train.py -s {data_dir}/{scene}/ -m {output_dir}/{scene}_nosky_depth  --port 8300 -i images_{factor} --use_prune_weight --use_depth --eval ",

            f"CUDA_VISIBLE_DEVICES={gpu} python render.py -s {data_dir}/{scene}/ -m {output_dir}/{scene}_nosky_depth --skip_train  -i images_{factor}",
            f"CUDA_VISIBLE_DEVICES={gpu} python metrics.py -m {output_dir}/{scene}_nosky_depth ",
