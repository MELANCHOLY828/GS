...
scenes = ["Auditorium", "M60", "Barn", "Caterpillar", "Family", "Francis", "Horse", "Ignatius", "Panther"]
factors = [ 4] * len(scenes)

normal_weights = [0.1] * len(scenes)
excluded_gpus = set([])

split = "scale"
# data_dir = '/data/liufengyi/data_gs/tandt_db/tandt'
data_dir = '/data/liufengyi/data_gs/tanks_and_temples/'

output_dir = "/data/liufengyi/res_gs/AtomGS_all"
jobs = list(zip(scenes, factors, normal_weights))

def train_scene(gpu, scene, factor, weight):
    cmds = [
            f"CUDA_VISIBLE_DEVICES={gpu} python train.py -s {data_dir}/{scene}/ -m {output_dir}/{scene}  --port 8000 -i images --use_prune_weight",
            # f"CUDA_VISIBLE_DEVICES={gpu} python render.py -s {data_dir}/{scene}/ -m {output_dir}/{scene}  --skip_train -i images",
            # f"CUDA_VISIBLE_DEVICES={gpu} python metrics.py -m {output_dir}/{scene}",
