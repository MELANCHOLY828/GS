...
def train_scene(gpu, scene, factor, weight):
    cmds = [
            f"CUDA_VISIBLE_DEVICES={gpu} python train.py -s {data_dir}/{scene}/ -m {output_dir}/{scene}_nosky_depth_dis  --port 8200 -i images_{factor} --use_prune_weight --regularization_from_iter 15000 --use_depth --eval",
